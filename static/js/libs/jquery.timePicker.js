...
(function($){
  ...
  $._timePicker = function(elm, settings) {
    ...
    var $tpDiv = $('<div class="timepicker'+ (settings.show24Hours ? '' : ' timepicker-12hours') +'"></div>');
    var $tpList = $('<ul></ul>');
    ...
    if ($(elm).parents('.modal').length > 0) {
        $tpDiv.appendTo('#lightbox-position').hide();
    } else {
    ...
    var showPicker = function() {
      ...
      if ($(elm).parents('.modal').length > 0) {
        var offset = $(elm).offset(),
            wrapperOffset = $('#lightbox-wrapper').offset();
            $tpDiv.css({
              top: offset.top + elm.offsetHeight - wrapperOffset.top,
              left: offset.left - wrapperOffset.left
            });
      } else {
          $tpDiv.css({
